...
// =============================================================================
// IPC Handlers — Role-based Web3 Access (for external MCP plugins)
//
// Same pattern as OS role. If an external MCP plugin is assigned the "web3"
// role, these handlers route calls to it. Built-in web3 tools (crypto prices,
// wallet) are handled by the ToolRegistry instead.
// =============================================================================

/** Resolve the connected Web3 plugin's server name, or throw */
function requireWeb3Server(): string {
  const plugin = pluginManager.byRole("web3");
  if (!plugin) throw new Error("No plugin has role 'web3'. Assign it in the MCP Servers panel.");
  if (!mcpClient.isConnected(plugin.name))
    throw new Error(`Web3 plugin "${plugin.name}" is not connected. Connect it in the MCP Servers panel.`);
  return plugin.name;
}

/** Status of the Web3 role: which plugin holds it and whether it's connected */
ipcMain.handle("web3:status", () => {
  const plugin = pluginManager.byRole("web3");
  if (!plugin) return { configured: false, connected: false };
  return {
    configured: true,
...
/** List all tools exposed by the Web3 plugin */
ipcMain.handle("web3:list-tools", async () => {
  try {
    const serverName = requireWeb3Server();
    const tools = await mcpClient.listTools(serverName);
    return { success: true, tools, serverName };
  } catch (error) {
    return { success: false, error: (error as Error).message, tools: [] };
...
/** Call any tool on the Web3 plugin */
ipcMain.handle(
  "web3:call",
  async (_event, toolName: string, args: Record<string, unknown> = {}) => {
    try {
