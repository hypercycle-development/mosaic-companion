...
| **Vault** | Boxes for notes and structured context — you choose which agents can read each box. In the published release contents are stored unencrypted on your machine and the Vault page says so, so treat them as readable by anything with access to the device. |
