...
import { Tab, INTERNAL_HOME_URL, INTERNAL_SETTINGS_URL } from '../types';
...
export const TabStrip: React.FC<TabStripProps> = ({ 
  ...
  const renderTabIcon = (tab: Tab) => {
    ...
    if (tab.history.present === INTERNAL_SETTINGS_URL) return <Settings size={14} />;
      ...
      {tabs.map((tab) => {
        ...
        return (
          <div
            ...
            `}
          >
            <span className={`flex items-center justify-center w-4 h-4 opacity-70 ${isActive ? 'text-indigo-500 dark:text-indigo-400' : ''}`}>
              {renderTabIcon(tab)}
              {/* Hidden fallback icon for when img fails */}
              {tab.favicon && !tab.isLoading && <Globe size={14} className="hidden" />}
            </span>
