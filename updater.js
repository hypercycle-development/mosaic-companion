...
export function checkForUpdates() {
  ...
  autoUpdater.checkForUpdates()
    .then((result) => {
      log('INFO', 'checkForUpdates resolved:', result ? {
...
export function manualCheckForUpdates() {
  ...
  autoUpdater
    .checkForUpdates()
    .then((result) => {
      log('INFO', 'Manual checkForUpdates resolved:', result ? {
        updateInfo: result.updateInfo,
        cancellationToken: !!result.cancellationToken
      } : 'null result');
...
autoUpdater.on("update-available", (info) => {
  ...
  dialog
    ...
    .then((result) => {
      log('INFO', `User clicked button index: ${result.response} (${result.response === 0 ? 'Download Now' : 'Later'})`);
      ...
      if (result.response === 0) {
        ...
        autoUpdater.downloadUpdate()
          ...
          .catch((downloadErr) => {
            ...
            log('ERROR', 'Full error object:', JSON.stringify(downloadErr, Object.getOwnPropertyNames(downloadErr), 2));
...
autoUpdater.on("download-progress", (progress) => {
  ...
  log('INFO', `>>> EVENT: download-progress - ${percent}% (${transferred}/${total} MB, ${speed} KB/s)`);
...
autoUpdater.on("update-downloaded", (info) => {
  ...
  dialog
    ...
    .then((result) => {
      log('INFO', `User clicked button index: ${result.response} (${result.response === 0 ? 'Restart Now' : 'Later'})`);
...
autoUpdater.on("error", (err) => {
  ...
  log('ERROR', 'Stack trace:', err.stack);
  log('ERROR', 'Full error:', JSON.stringify(err, Object.getOwnPropertyNames(err), 2));
  log('ERROR', '========================================');
