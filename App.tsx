...
import { INTERNAL_HOME_URL, Tab } from './types';
...
function App() {
  ...
  const [theme, setTheme] = useState<'light' | 'dark'>(() => {
    return (localStorage.getItem('browser_theme') as 'light' | 'dark') || 'dark';
  });
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-gray-900/50 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex-1 relative overflow-hidden bg-gray-900">
          ...
          {tabs.map(tab => (
            <div 
              key={tab.id} 
              className={`absolute inset-0 w-full h-full flex flex-col ${tab.id === activeTabId ? 'z-10' : 'z-0 invisible'}`}
            >
               <ContentArea 
