 ...
 */
async function runAgent(mcp, serverName, userQuery, provider = "openai") {
    console.log("\n🤖 Starting agent with query:", userQuery);
    console.log("─".repeat(50));
    // Get available tools from MCP server
    const tools = await mcp.listTools(serverName);
    console.log(`📦 Loaded ${tools.length} tools from ${serverName}:`, tools.map((t) => t.name).join(", "));
    ...
    while (iterations < maxIterations) {
        ...
        for (const toolCall of toolCalls) {
            ...
            try {
                ...
                console.log(`   ← Result: ${resultText.slice(0, 100)}${resultText.length > 100 ? "..." : ""}`);
...
async function main() {
    ...
    if (!OPENAI_API_KEY && !ANTHROPIC_API_KEY) {
        console.log("⚠️  No API keys configured. Set OPENAI_API_KEY or ANTHROPIC_API_KEY");
    ...
    try {
        ...
        if (OPENAI_API_KEY || ANTHROPIC_API_KEY) {
            ...
            const response = await runAgent(mcp, serverName, "What is the current time in Tokyo? Also, fetch the homepage of example.com and summarize it.", provider);
            console.log("\n📝 Final Response:");
