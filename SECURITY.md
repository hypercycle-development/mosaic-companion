...
- Vault box contents are encrypted at rest using the operating system's own secure storage — Keychain on macOS, DPAPI on Windows, the Secret Service on Linux. Protection therefore depends on what the machine provides, and the Vault page states which of three states applies rather than assuming the best one. Where a real backend is present, contents are protected and a copied file cannot be read elsewhere. On a Linux system falling back to `basic_text`, contents are obfuscated but **not** protected — that backend stores its key in the clear. Where no backend is available, or a keychain prompt is declined, new contents are written as plaintext and the page says so. Per-agent access controls are enforced within the application and are a separate thing from encryption; they do not protect the files on disk.
