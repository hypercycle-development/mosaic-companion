...
- The Vault is an early component. Boxes hold user-entered text stored as local JSON, unencrypted at rest, and the Vault page says so. Per-agent access controls are real and enforced within the application, but they do not protect the files on disk — anyone who can read your files, or a backup of them, can read your boxes. Encryption at rest is built and expected in the next release; until it ships, treat the contents as readable by anything with access to the machine.
