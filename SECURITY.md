...
- Vault box contents are stored as local JSON, **unencrypted at rest**, and the Vault page says so. Per-agent access controls are enforced within the application and are a separate thing from encryption; they do not protect the files on disk — anyone who can read your files, or a backup of them, can read your boxes. Do not put anything sensitive in a box. Encryption at rest using the operating system's secure storage is built and is **not yet in a published release**; when it ships, protection will depend on what the machine provides and the Vault page will report which of three states applies.
