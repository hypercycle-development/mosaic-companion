...
Reports are received by all three maintainers ([MAINTAINERS.md](MAINTAINERS.md)), so a report does not wait on one person being available.
...
- Add-ons are distributed through a signed catalogue. The application trusts a single pinned publisher key and refuses anything it cannot verify against it. A registry carries a monotonic sequence number, and one older than the highest already verified is rejected, so a previously valid registry cannot be replayed to erase a withdrawal. A withdrawn add-on stops running on the next catalogue sync, without needing an application update. Two limits are worth stating plainly: main-process add-on code runs unsandboxed and outside the permission model, so it is restricted to an allowlist rather than generally available; and a signature establishes who published an add-on, never that the add-on is safe — review addresses the second, and review is human.
- Vault box contents. **In the current published release (v0.1.12) they are stored as local JSON, unencrypted at rest**, and the Vault page says so — anyone who can read your files, or a backup of them, can read your boxes. Do not put anything sensitive in a box on a published build. Encryption at rest using the operating system's own secure storage — Keychain on macOS, DPAPI on Windows, the Secret Service on Linux — is built and enabled on `main`, and is **not yet in any published release**. Where it is active, protection depends on what the machine provides and the Vault page reports which of three states applies: protected; obfuscated but **not** protected on a Linux `basic_text` fallback, which stores its key in the clear; or plaintext where no backend is available or a keychain prompt is declined. Per-agent access controls are enforced within the application and are a separate thing from encryption; they do not protect the files on disk.
